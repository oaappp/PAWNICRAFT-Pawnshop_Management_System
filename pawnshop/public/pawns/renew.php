...
if (isset($_GET['ticket'])) {
    ...
    if ($ticket !== '') {
        ...
        } elseif (!in_array($transaction['status'], ['active','renewed','expired'], true)) {
            $error = 'This ticket cannot be renewed (status: ' . $transaction['status'] . ').';
            $transaction = null;
        } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['transaction_id'])) {
    ...
    if (!$transaction) {
        ...
        $interest_to_pay = $charges['interest_total'];
        $penalty_to_pay  = $charges['penalty'];
        $total_to_pay    = $interest_to_pay + $penalty_to_pay;

        $termDays  = (int)get_setting($db, 'default_loan_term_days', 30);
        $graceDays = (int)get_setting($db, 'grace_period_days', 15);

        $old_maturity_date = $transaction['maturity_date'];
        $new_maturity_date = date('Y-m-d', strtotime("+{$termDays} days", strtotime($old_maturity_date)));
        $new_grace_end     = date('Y-m-d', strtotime("+{$graceDays} days", strtotime($new_maturity_date)));

        try {
            $db->beginTransaction();

            $receipt_number = 'RN-' . date('YmdHis') . '-' . random_int(1000, 9999);
            ...
            $stmtRen = $db->prepare(
                "INSERT INTO renewals (
                    transaction_id, renewal_date, old_maturity_date,
                    new_maturity_date, interest_paid, processed_by
                 ) VALUES (
                    :tid, CURDATE(), :old_md,
                    :new_md, :int_paid, :uid
                 )"
            ...
            $stmtRen->execute([
                ':tid'      => $transaction_id,
                ':old_md'   => $old_maturity_date,
                ':new_md'   => $new_maturity_date,
                ':int_paid' => $interest_to_pay + $penalty_to_pay,
                ':uid'      => $_SESSION['user_id'],
            ]);

            $stmtUp = $db->prepare(
                "UPDATE pawn_transactions
                 SET maturity_date = :new_md,
                     grace_period_end = :new_gd,
                     status = 'renewed'
                 WHERE transaction_id = :id"
            );
            $stmtUp->execute([
                ':new_md' => $new_maturity_date,
                ':new_gd' => $new_grace_end,
                ':id'     => $transaction_id,
            ]);
            ...
            $db->prepare(
                "INSERT INTO audit_logs
                 ...
                 VALUES
                   (:uid, 'update', 'pawn_transactions', :rid, :newv, :ip)"
            )->execute([
                ':uid'  => $_SESSION['user_id'],
                ':rid'  => $transaction_id,
                ':newv' => json_encode([
            ...
            $db->rollBack();
            $error = 'Error during renewal: ' . $e->getMessage();
        }
    }
}
...
<div class="d-flex justify-content-between align-items-center mb-3">
    <div>
        <h3 class="mb-0 text-white">Renewal</h3>
        <small class="text-muted">
            Extend the pawn period by paying only the interest and penalties. Principal remains unchanged.
        </small>
    </div>
</div>
...
<div class="card mb-4">
    ...
    <div class="card-body">
        ...
        <small class="text-muted">
            Enter the ticket to see interest due and the new maturity date after renewal.
        </small>
    </div>
</div>
            ...
            <div class="card mb-3">
                ...
                <div class="card-body">
                    ...
                    <div class="row mb-2">
                        ...
                        <div class="col-md-4">
                            <p class="mb-1 small text-muted">Interest to pay</p>
                            <div>
                    ...
                    <div class="row mb-2">
                        <div class="col-md-4">
                            <p class="mb-1 small text-muted">Total for renewal</p>
                            <div class="h5 text-primary">
                                <?php echo number_format($charges['interest_total'] + $charges['penalty'],2); ?>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <p class="mb-1 small text-muted">Current Maturity Date</p>
                            <div>
                    ...
                    <small class="text-muted">
                        After renewal, the maturity date and grace period will be extended based on your configured loan term.
                    </small>
                </div>
            </div>
                    ...
                    <form method="post" class="row g-3 align-items-end">
                        ...
                        <div class="col-md-4 text-md-end">
                            <button class="btn btn-teal">
                                <i class="bi bi-arrow-repeat me-1"></i> Confirm Renewal
                            </button>
                        </div>
                    </form>
                </div>
...
<?php else: ?>
    <p class="text-muted text-center">
        Search for a pawn ticket to calculate renewal interest and penalties.
    </p>
<?php endif; ?>
